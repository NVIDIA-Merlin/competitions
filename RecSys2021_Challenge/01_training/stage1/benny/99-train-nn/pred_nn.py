...
from nn_utils import CustomDataset, compute_rce, process_epoch, preprocess_nn, compute_rce_fast, process_epoch_pred
from utils import *
...
valid_files = sorted(glob.glob('/raid/recsys2021_valid_pre_TEnn_opt/*'))
print(valid_files)
...
del df_valid
gc.collect()
modelfiles = ['/raid/nnmodels/model_last.pth']
results = []
...
for modelfile in modelfiles:
    ...
    valid_out = process_epoch_pred(valid_loader, simplemodel, False, y_list=[], y_pred_list=[])
    ...
    pickle.dump(results, open('results_nn_2_val.pickle', 'wb'))
    pickle.dump([valid_out_pred, valid_out_gt, valid_out[4]], open('model_pred_opt2.pickle', 'wb'))
    del simplemodel
